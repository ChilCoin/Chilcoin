...
bool GetMyExternalIP(CNetAddr& ipRet)
{
    ...
    for (int nHost = 1; nHost <= 2; nHost++)
    {
        ...
        {
            addrConnect = CService("216.146.39.70", 80); // www.showmyip.com

            if (nLookup == 1)
            {
                CService addrIP("checkip.dyn.com/", 80, true);
                if (addrIP.IsValid())
                    addrConnect = addrIP;
            }
            ...
            pszKeyword = "Address:"; // Returns just IP address
        }
...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "Chilcoin " + FormatFullVersion();
...
static const char *strMainNetDNSSeed[][2] = {
    {"chilcoin.cl", "seed.chilcoin.cl"},
    {NULL, NULL}
};

static const char *strTestNetDNSSeed[][2] = {
    {"chilcoin.cl", "test.chilcoin.cl"},
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Chilcoin is probably already running."), addrBind.ToString().c_str());
        else
